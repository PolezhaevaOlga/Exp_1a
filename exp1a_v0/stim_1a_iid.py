...
pre_test_phase = False # If you want to run a pre-test of 10 random trials. If non = False
...
iid_csv= pd.read_csv('iid_all.csv', header=None, sep = ";").to_numpy()


dot_radius = 0.3 # Radius of each dot in deg
time_intertrial = 1000 # in msec

display_dist = 60  # distance from the human eye to the stimulus, measured in in cm
frames_per_second = 55 # my monotor
display_width = 54 #cm horizontal size of my screen (labo HP = 54cm and pc = 34 cm)
N = 150 # nomnre of dots total

# stim_array = np.flip(iid_csv,axis=1)
stim_array = iid_csv
# stim_array = iid_csv[:, np.random.RandomState(seed=43).permutation(iid_csv.shape[1])]
...
pygame.display.set_caption('Exp_1a')
...
class resulaj(dot):
    ...
    def run(self):
        ...
        for k in range(n_trials):
            ...
            self.index_old.append(int(stim_array[0,k]))
            self.index_traj.append(int(stim_array[3,k]))
            self.r = stim_array[4:,k]
            self.f = functions.search_f(stim_array[:,k])
            
            if stim_array[1,k] == 5 :
                self.path_trial.append('5%')
                print(f"cond last 5")
            else:
                self.path_trial.append('15%')
                print(f"cond last 15")
            if np.round(np.mean(stim_array[4:-(self.f),k]),3) <=0 :
                self.mean_side_trial.append('left')
                print(f"mean : {np.mean(stim_array[ :,k])}")
            else:
                self.mean_side_trial.append('right')
                print(f"mean : {np.mean(stim_array[4:,k])}")

            # self.f =stim_array[2,k]
            ...
            self.mean_val.append(np.round(np.mean(stim_array[4:-(self.f),k]),3))
            self.stim_non_zero =stim_array[4:-(self.f), k]
            
            last_5_pursent = int(len(self.stim_non_zero)*0.05)
            last_15_pursent = int(len(self.stim_non_zero)*0.15)

            self.mean_last5.append(np.round(np.mean(self.stim_non_zero[-last_5_pursent:]),3))
            self.mean_last15.append(np.round(np.mean(self.stim_non_zero[-last_15_pursent:]),3))

            # self.mean_last15.append(np.round(np.mean(self.r_non_zero[-int(len(self.r_non_zero*0.15)):]),3))
            
            self.sd_trial.append(np.round(np.std(stim_array[4:-(self.f), k]),3))
            self.to_be_pred_abs_d.append(np.round(stim_array[-(self.f+1),k],3))
    ...
    def resulaj_trial(self, trial_num):
        ...
        print(f'f= {self.f}')
        print(f"last visible {np.round(stim_array[-(self.f+1),trial_num],3)}")
        
        print(f"befor last visible {np.round(stim_array[-(self.f+2),trial_num],3)}")
       ...
       # start of the trial demonstration of the fixation cross
        ...
        while not self.intertrial_period_over:
            ...
            if self.count_dot < len(self.r)-(self.f+1) and not self.stimulus_over:
                self.sprite_group.update(self.r[self.count_dot],self.count_dot)
                self.sprite_group.draw(screen)
                self.count_dot = self.count_dot + 1

            elif self.count_dot == len(self.r)-(self.f+1): # last visible dot parametres
            ...
            elif self.count_dot == (len(self.r)-(self.f+1)+1) :
        ...
        self.trial_dict['index_old'] = self.index_old
        # trajectory parametres
        self.trial_dict['mean_side'] = self.mean_side_trial
        self.trial_dict['mean_val']=self.mean_val 
        self.trial_dict['mean_last5']=self.mean_last5
        self.trial_dict['mean_last15']=self.mean_last15
