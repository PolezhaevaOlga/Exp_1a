...
# rdw_csv = pd.read_csv('rdw_ind.csv', header=None, sep = ";").to_numpy()

# iid_csv= pd.read_csv(stim_csv[0], header=None).to_numpy()
# rdw_csv = pd.read_csv(stim_csv[1], header=None).to_numpy()
stim_array = iid_csv[:, np.random.RandomState(seed=43).permutation(iid_csv.shape[1])][:,0:10]
...
class resulaj(dot):
    ...
    def run(self):               
        ...
        for k in range(n_trials):    
            ...
            self.f = functions.search_f(stim_array[:,k])+1                              
